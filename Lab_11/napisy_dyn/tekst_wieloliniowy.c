...
int main( void )
{
  ...
  for(int nr_linii=0; nr_linii<MAX_N_L; nr_linii++){
    ...
    wczytaj_linie_dyn_str(&tekst_wieloliniowy_tab_str[nr_linii], zakres);
  ...
  for(int nr_linii=0; nr_linii<MAX_N_L; nr_linii++){
    ...
    rozmiar_str_danych += tekst_wieloliniowy_tab_str[nr_linii].dlugosc_linii*sizeof(char);
  }
  ...
  for(int nr_linii=0; nr_linii<MAX_N_L; nr_linii++){
    ...
    free (tekst_wieloliniowy_tab_str[nr_linii].dlugosc_linii);
  }
...
int wczytaj_linie_dyn(char** linia, int max_dlugosc_linii){
  ...
  linia_max[i] = '\0'; // od 0 do i - czyli i+1 elementów tablicy
  // co stanie się w dalszej części programu jeśli nie umieścimy \0 w tablicy?
  


  *linia=malloc((i+1)*sizeof(char));
  
  int j=0;
  char temp;
  while( (temp=linia_max[j])!='\0' ){ 
    (*linia)[j]=temp; 
    j++; 
  }
  (*linia)[j] = '\0';
  ...
  return(j); // liczymy tylko znaki, bez \0 !!!
}
...
int wczytaj_linie_dyn_str(struct linia_tekstu * linia_str_wsk, int max_dlugosc_linii){
  ...
  linia_str_wsk->znaki=malloc((i+1)*sizeof(char));
  
  int j=0;
  char temp;
  while( (temp=linia_max[j])!='\0' ){ 
    (linia_str_wsk->znaki)[j]=temp; 
    j++; 
  }
  (linia_str_wsk->znaki)[j] = '\0';

  linia_str_wsk->dlugosc_linii=j;
